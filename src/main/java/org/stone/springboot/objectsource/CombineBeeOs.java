 ...
 * @author Chris Liao
 */
public class CombineBeeOs {
    private final ThreadLocal<SpringBootBeeOs> osThreadLocal;
    private boolean isClosed = false;

    CombineBeeOs(ThreadLocal<SpringBootBeeOs> osThreadLocal) {
    ...
    private SpringBootBeeOs getCurrentObjectSource() throws Exception {
        if (isClosed) throw new Exception("ObjectSource has closed");
        SpringBootBeeOs os = osThreadLocal.get();
