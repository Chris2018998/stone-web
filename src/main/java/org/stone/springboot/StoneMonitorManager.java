...
public final class StoneMonitorManager {
    ...
    //                                     1: manager init(1)                                                        //
    //***************************************************************************************************************//
    void init(StoneMonitorConfig config) {
        if (sqlTrace = config.isSqlTrace()) {
            ...
            this.timerExecutor.scheduleAtFixedRate(new SqlTraceTimeoutTask(this, config.getSqlTraceTimeout()),
            ...
            if (!isBlank(redisHost)) {//send datasource info to redis
                ...
                this.timerExecutor.scheduleAtFixedRate(new RedisPushTask(pool, expireSeconds), 0, config.getRedisSendPeriod(), MILLISECONDS);
            }
        }
    }
    ...
    private static final class SqlTraceTimeoutTask implements Runnable {
        ...
        private final StoneMonitorManager manager;
        private final LinkedList<StatementTrace> sqlAlertTempList;

        SqlTraceTimeoutTask(StoneMonitorManager manager, long sqlTraceTimeout) {
