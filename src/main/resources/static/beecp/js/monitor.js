$(function () {
    ...
    var dsURL = getContextPath() + '/stone/beecp/getDataSourceList';
    var osURL = getContextPath() + '/stone/beeop/getDataSourceList';
    var sqlURL = getContextPath() + '/stone/beecp/getSqlTraceList';
    ...
    $("#os_refresh_button").click(function () {
        getOsListFromServer();
        alert(refreshMsg);
    });
    ...
    $("#os_timer_button").click(function () {
        if (osRefreshTask != null) {//stop
            clearInterval(osRefreshTask);
            osRefreshTask = null;
            var name = (language == 'cn') ? '启动定时' : 'Run Timer';
            $("#os_timer_button").val(name)
        } else {//run
            osRefreshTask = setInterval(getOsListFromServer, $("#os_refresh_interval").val());
            var name = (language == 'cn') ? '停止定时' : 'Stop Timer';
            $("#os_timer_button").val(name);
        }
    });
    ...
    $("#os_refresh_interval").click(function () {
        if (osRefreshTask != null) {
            clearInterval(osRefreshTask);
            osRefreshTask = setInterval(getOsListFromServer, $("#os_refresh_interval").val());
        }
    });
    ...
    function getSqlListFromServer() {
        $.ajax({
            ...
            success: function (data) {
                if (data.code == 3) {
                    window.location.href = getContextPath() + "/stone/login.html";
                } else if (data.code == 2) {
    ...
    function getOsListFromServer() {
        $.ajax({
            ...
            success: function (data) {
                console.info(data);
                $("#os_monitorTable tr:not(:first)").remove();
                if (data) {
                    if (data.code == 3) {
                        window.location.href = getContextPath() + "/stone/login.html";
                    } else if (data.code == 2) {
                        ...
                        $.each(data.result,
                            function (i, element) {
                                ...
                                var tableHtml = "<tr>" + "<td>" + element.osId + "</td>"
                                    + "<td>" + mode + "</td>" + "<td>" + state + "</td>"
                                    + "<td>" + element.poolMaxSize + "</td>"
                                    + "<td>" + element.idleSize + "</td>"
                                    + "<td>" + element.usingSize + "</td>"
